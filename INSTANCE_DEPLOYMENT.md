...
| `auth@v2` step fails with `unauthorized_client` | `WIF_PROVIDER`/`PROVISIONER_SA` variables wrong, or `GITHUB_REPO` was passed to `setup_wif.sh` with different casing than the real repo name. |
